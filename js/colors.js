...
(function() {
  ...
  module.exports['Ri'] = function(bi, fi) {
    return R(bi) + BG_FG_COLORS['R' + bi][FG_COLORS[fi - 1]];
  };

  module.exports['Gi'] = function(bi, fi) {
    return G(bi) + BG_FG_COLORS['G' + bi][FG_COLORS[fi - 1]];
  };

  module.exports['Bi'] = function(bi, fi) {
    return B(bi) + BG_FG_COLORS['B' + bi][FG_COLORS[fi - 1]];
  };

  module.exports['Yi'] = function(bi, fi) {
    return Y(bi) + BG_FG_COLORS['Y' + bi][FG_COLORS[fi - 1]];
  };

  module.exports['Mi'] = function(bi, fi) {
    return M(bi) + BG_FG_COLORS['M' + bi][FG_COLORS[fi - 1]];
  };

  module.exports['Ci'] = function(bi, fi) {
    return C(bi) + BG_FG_COLORS['C' + bi][FG_COLORS[fi - 1]];
  };

  module.exports['Wi'] = function(bi, fi) {
    return W((bi - 1) * 3 + 2) + BG_FG_COLORS['W' + bi][FG_COLORS[fi - 1]];
  };
  ...
  module.exports.show = function() {
    var len3, len4, o, p, q, s;
    for (o = 0, len3 = BG_COLORS.length; o < len3; o++) {
      bg = BG_COLORS[o];
      for (bi = p = 1; p < 9; bi = ++p) {
        s = colors.reset;
        bn = bg + bi;
        s += eval(bg.toLowerCase())(bi) + (bg + bi) + " " + eval(bn);
        for (q = 0, len4 = FG_COLORS.length; q < len4; q++) {
          fg = FG_COLORS[q];
          fn = bn + fg;
